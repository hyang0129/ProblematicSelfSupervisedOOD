    ...
    def __init__(self, fn, ask=True, local_rank=0):
        ...
        if self.local_rank == 0:
            ...
            if len(os.listdir(logdir)) != 0 and ask:
                ans = 'y' #input("log_dir is not empty. All data inside log_dir will be deleted. Will you proceed [y/N]? ")
                if ans in ['y', 'Y']:
...
def save_checkpoint(run, epoch, model_state, optim_state, logdir):

    path = os.path.join(logdir, str(run))
    if not os.path.exists(path):
        os.mkdir(path)
    last_model = os.path.join(logdir, str(run), 'last.model')
    last_optim = os.path.join(logdir, str(run), 'last.optim')
    last_config = os.path.join(logdir, str(run), 'last.config')
...
def load_linear_checkpoint(logdir, mode='last'):
    if mode == 'last':
        linear_optim_path = os.path.join(logdir, str(run) , 'last.linear_optim')
    elif mode == 'best':
        linear_optim_path = os.path.join(logdir, str(run), 'best.linear_optim')
    else:
        raise NotImplementedError()
...
def save_linear_checkpoint(run, linear_optim_state, logdir):
    path = os.path.join(logdir, str(run))
    if not os.path.exists(path):
        os.mkdir(path)
