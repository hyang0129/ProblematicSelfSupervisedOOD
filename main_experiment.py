...
@click.option('--random_state', default = 42, type = int, help = 'affects which classes are assigned as OOD and ID')
@click.option('--results_dir', default = 'results', help = 'directory to put results, which will tree into results/dataset/method')
@click.option('--device', default = 'cuda:0', help = 'This is meant to run on an A100 due to gpu memory requirements')
def main(arch,
        dataset,
        normalize,
        batch_size,
    ...
    model, criterion, optimizer, lr_scheduler = get_model(args, in_distro, args.device, train_loader)

    do_train = True

    if do_train:
        model = train(args, model, optimizer, criterion, lr_scheduler, args.device, train_loader, test_loader, )
