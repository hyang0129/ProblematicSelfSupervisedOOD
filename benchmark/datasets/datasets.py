        ...
        if dataset_name == "food101":
            ...
            if not os.path.exists("food_label_index.csv"):
                ...
                for i, data in tqdm(enumerate(datasource["train"])):
                    label_info.append(
                        {"split": "train", "index": i, "label": data["label"]}
                    )

                for i, data in tqdm(enumerate(datasource["validation"])):
                    label_info.append(
                        {"split": "val", "index": i, "label": data["label"]}
                    )
            ...
            test_data = torchvision.datasets.StanfordCars(
                root="data", download=True, split="test"
            )
...
def get_dataloaders(
    ...
    train_set = OODDataset(
        ...
        rotnet=args.training_mode == "RotNet",
        random_state=random_state,
    )
    test_set = OODDataset(
        dataset_name, split="Test", transform=transform_test, random_state=random_state
    )
    ood_set = OODDataset(
        dataset_name, split="Out", transform=transform_test, random_state=random_state
    )
    ...
    invTrans = transforms.Compose(
        [
            transforms.Normalize(
                mean=[0.0, 0.0, 0.0], std=1 / np.array([0.2675, 0.2565, 0.2761])
            ),
            transforms.Normalize(
                mean=np.array([0.5071, 0.4867, 0.4408]) * -1, std=[1.0, 1.0, 1.0]
            ),
        ]
    )
