...
def get_eval_results_knn(ftrain, ftest, food, args):
    ...
    for i in tqdm(range(iters)):
        test_scores.append(
            cp.apply_along_axis(f, 1, cp.array(ftest[i * n : (1 + i) * n])).get()
        )
    ...
    for i in tqdm(range(iters)):
        ood_scores.append(
            cp.apply_along_axis(f, 1, cp.array(food[i * n : (1 + i) * n])).get()
        )
...
def run_evaluation(
    ...
    model.load_state_dict(
        ...
        else ckpt_dict["model"]
    )

    if args.training_mode in ["SimCLR", "RotNet"]:
        features_train, labels_train = get_features(model.encoder, train_loader, device)

        features_test, _ = get_features(model.encoder, test_loader, device=device)
        logger.info(
            "In-distribution features shape: ",
